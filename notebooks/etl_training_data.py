...
# Initialize the projection transformer (for accurate distance calculations)
import shapely


wgs84 = pyproj.CRS('EPSG:4326')
utm_crs = pyproj.CRS('EPSG:3857')  # Web Mercator Projection (Meters)
...
for idx in tqdm(range(NUM_POSSIBLY_EMPTY_AREAS), desc="Generating Possibly Empty Areas"):
    ...
    # Random distance between 10 km and 100 km
    ...
    input_area_utm = input_area_to_utm = input_area  # Initialize

    if input_areas.crs != utm_crs:
        # Transform to UTM
        input_area_utm = input_areas.to_crs(utm_crs).iloc[0].geometry

    # Move the area north by the random distance
    # In UTM, moving north means increasing the y-coordinate
    ...
    try:
        # Use current date range for images
        random_year = pd.Timestamp(str(random.randint(2020, 2024)))

        input_image = input_area_to_input_image(
            input_area=moved_area_wgs84,
            image_data_start_date=random_year,
            image_data_end_date=random_year,
